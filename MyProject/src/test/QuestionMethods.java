...
{	
	...
	*/
	public static List<Question> searchQuestions(String query, String domain)
	{	
		List<Question> lq = new ArrayList<Question>();
		
		try(Connection con = DbConnection.createConnection())
		{
			PreparedStatement stmt = con.prepareStatement("SELECT so_questions.id, so_questions.subject, so_questions.domain, so_questions.content, so_users.name FROM so_questions, so_users WHERE so_users.id = so_questions.user_id AND so_questions.domain = ? AND so_questions.subject LIKE ?");
